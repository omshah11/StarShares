...
const BarChart = ({ ownedStockList }) => {
  ...
  useEffect(() => {
    ...
    const dataValues = ownedStockList.map(stock => stock.cost * stock.quantity)
    myChartRef.current = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [{
          label: 'Stocks owned X Stock Value',
          data: dataValues,
          backgroundColor: [
            'rgba(255, 99, 132, 0.2)',
            'rgba(54, 162, 235, 0.2)',
  ...
  return (
    <div className='bg-white mx-2 mr-10 border-2 border-gray-600 rounded-xl drop-shadow-md justify-center items-center mt-4'>
      <div className="flex flex-col justify-center px-auto">
        <h2 className="flex justify-center text-xl font-semibold my-4">Bar Chart</h2>
