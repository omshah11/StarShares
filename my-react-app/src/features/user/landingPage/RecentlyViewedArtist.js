  ...
  return (
    <div className="mx-20 m-auto grid grid-cols-5 gap-4 gap-x-4">
      {artistDetailsList.map(artistDetails => (
        <div key={artistDetails.id} className="card-container" onClick={() => handleStockBtn(artistDetails.name, artistDetails.id)}>
          <div className="block rounded-lg bg-white shadow-secondary-1 dark:bg-surface-dark">
            <div
              className="rounded-t-lg"
              style={{
                height: '400px',
                backgroundImage: `url(${artistDetails.images && artistDetails.images.length > 0 ? artistDetails.images[0]?.url : ''})`, // Check if artistDetails.images exists and has length greater than 0
            ...
            <div className="p-6 text-surface dark:text-white">
              ...
              <button
                ...
                data-twe-ripple-color="light" onClick={() => navigate(artistDetails.external_urls.spotify)}>
                Spotify
              </button>
            </div>
          </div>
